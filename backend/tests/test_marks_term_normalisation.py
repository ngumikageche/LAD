...
TERMS = [_Term("Term 1 2026"), _Term("Term 2 2026"), _Term("Term 3 2026")]


def test_exact_label_is_kept():
    errors = []
    assert _normalise_term("Term 1 2026", _Assessment("Term 2 2026"), TERMS, 2, errors) == "Term 1 2026"
    assert errors == []
...
def test_blank_falls_back_to_the_assessments_term_without_complaint():
    errors = []
    assert _normalise_term("", _Assessment("Term 2 2026"), TERMS, 2, errors) == "Term 2 2026"
    assert _normalise_term(None, _Assessment("Term 2 2026"), TERMS, 2, errors) == "Term 2 2026"
    assert errors == []
...
def test_unknown_label_is_replaced_and_reported():
    ...
    errors = []
    assert _normalise_term("TERM1", _Assessment("Term 2 2026"), TERMS, 7, errors) == "Term 2 2026"
    assert len(errors) == 1
...
def test_unknown_label_with_no_assessment_term_is_kept_but_flagged():
    ...
    errors = []
    assert _normalise_term("TERM1", _Assessment(None), TERMS, 3, errors) == "TERM1"
    assert len(errors) == 1
...
def test_punctuation_only_difference_is_resolved():
    # The real label that hid 8 of a trainer's marks.
    errors = []
    assert _normalise_term("Term 2, 2026", _Assessment(None), TERMS, 5, errors) == "Term 2 2026"
    assert errors == []


def test_bare_term_number_is_resolved():
    # A spreadsheet column of term numbers — the label on 40 more marks.
    errors = []
    assert _normalise_term("1", _Assessment(None), TERMS, 6, errors) == "Term 1 2026"
    assert _normalise_term("2", _Assessment(None), TERMS, 7, errors) == "Term 2 2026"
    assert errors == []


def test_an_ambiguous_number_is_not_guessed():
    # "2026" appears in all three names, so it means nothing on its own.
    errors = []
    assert _normalise_term("2026", _Assessment(None), TERMS, 8, errors) == "2026"
    assert len(errors) == 1
