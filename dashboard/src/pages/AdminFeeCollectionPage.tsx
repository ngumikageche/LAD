  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 print:bg-white print:p-0">
      ...
      <div className="max-w-5xl mx-auto mb-4 flex items-center gap-3 print:hidden">
        <h1 className="text-xl font-bold text-gray-900 flex-1">Fee Collection Report</h1>
      ...
      <div className="max-w-5xl mx-auto bg-white shadow-lg print:shadow-none" style={{ padding: '16mm' }}>
        ...
        <div className="text-center border-b-2 border-gray-800 pb-4 mb-6">
          ...
          <h2 className="text-lg font-bold text-gray-800 mt-2 uppercase">Fee Collection Report</h2>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="p-5 bg-gray-50 border border-gray-200 rounded-lg text-center">
            ...
            <p className="text-2xl font-bold text-gray-900">{fmt(data.summary.total_billed)}</p>
          </div>
          ...
          <div className={`p-5 rounded-lg text-center border ${data.summary.outstanding > 0 ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}>
            <p className={`text-xs uppercase mb-1 ${data.summary.outstanding > 0 ? 'text-red-600' : 'text-gray-500'}`}>Outstanding</p>
            <p className={`text-2xl font-bold ${data.summary.outstanding > 0 ? 'text-red-700' : 'text-gray-900'}`}>{fmt(data.summary.outstanding)}</p>
        ...
        <h3 className="font-bold text-gray-800 mb-3 uppercase text-sm">Collection Rate by Course</h3>
        <div className="mb-8 space-y-2">
          {data.by_course.length === 0 ? (
            ...
            <div key={row.course_id} className="flex items-center gap-3">
              <span className="text-sm text-gray-700 w-48 truncate">{row.course_name}</span>
              <span className="text-xs text-gray-500 w-16 text-right">{row.student_count} students</span>
              ...
              <span className="text-xs font-semibold text-gray-700 w-12 text-right">{row.collection_rate}%</span>
            </div>
          ))}
        </div>
        ...
        {data.defaulters.length > 0 && (
          <div>
            <button
              ...
              className="flex items-center gap-2 font-bold text-gray-800 uppercase text-sm mb-3 print:hidden"
            >
              {showDefaulters ? '▼' : '▶'} Outstanding Balances ({data.defaulters.length} students)
            </button>
            <h3 className="font-bold text-gray-800 uppercase text-sm mb-3 hidden print:block">
                ...
                <tbody>
                  {data.defaulters.map((d, i) => (
                    <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-3 py-2 text-gray-600">{d.course_name}</td>
