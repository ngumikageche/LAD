...
const InstitutionsPage = () => {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const data = await apiRequest<Institution[]>('/institutions', { token });
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (formState.id) {
        await apiRequest(`/institutions/${formState.id}`, {
          method: 'PUT',
          token,
          body: payload,
        });
      } else {
        await apiRequest('/institutions', {
          method: 'POST',
          token,
          body: payload,
        });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Institutions</h1>
          <p className="text-sm text-gray-500">Manage institutions and campuses.</p>
        </div>
        {user?.permissions?.['institutions.create'] || user?.permissions?.['*'] ? (
          <button
            onClick={openCreate}
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        {isLoading ? (
          <div className="p-6 text-sm text-gray-600">Loading institutions...</div>
        ) : error ? (
            ...
            <table className="w-full text-left">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider">Location</th>
                  ...
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-gray-600">
                      <div className="flex items-center gap-3">
                        {user?.permissions?.['institutions.update'] || user?.permissions?.['*'] ? (
                          <button
                        ...
                        {user?.permissions?.['institutions.delete'] || user?.permissions?.['*'] ? (
                          <button
