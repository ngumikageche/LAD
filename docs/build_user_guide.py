...
<section class="first">
  ...
  <ol class="toc">
    ...
    <li>How the record fits together <span class="s">— structure, terms, marks, and mastery</span></li>
    <li>For learners <span class="s">— dashboard, attendance, marks, subjects, portfolio</span></li>
...
<section>
  ...
  <h2>How the record fits together</h2>

  <p>Every number LAD shows — an average, a pass rate, a mastery percentage — is computed
  from a small set of records that reference each other. Knowing the chain explains most
  of what the dashboards do, and most of what looks odd when a link is missing.</p>

  <h3>Structure, and where everything hangs</h3>
  <p>The academic hierarchy runs
  <code>Institution → Department → Course → Module → Subject</code>. Learners enrol on a
  course and are attached to subjects; trainers are assigned subjects. Everything else —
  marks, attendance, documents, reports — hangs off those attachments, which is why scope
  (section 2) is resolved by walking this chain.</p>

  <h3>Terms</h3>
  <p>Marks carry a <strong>term label</strong>, and every report groups by it. Exactly one
  term is active at a time and reports default to it — so a mark filed under a different
  term, or under a mistyped label, silently drops off the default view. Keeping term labels
  consistent is what keeps the term trend and the report tables telling one story.</p>

  <h3>Marks</h3>
  <p>A mark always belongs to an <strong>assessment</strong> — a specific piece of marked
  work with a total, a pass mark, a type (quiz, CAT, exam…), and a term. Marks are stored
  as raw scores but compared as <strong>percentages of their assessment's total</strong>,
  so a paper out of 40 and one out of 100 sit fairly in the same average. The pass
  threshold is 50%.</p>

  <h3 class="keep">Competencies, and how mastery is measured</h3>
  <p>A <strong>competency</strong> is a unit of skill defined on a module: a name, what the
  learner should be able to do, and a <strong>mastery threshold</strong> — the percentage
  that counts as having mastered it. The definition is curriculum, maintained by an
  administrator; trainers grade against it but do not edit it.</p>
  <p>A competency never receives a mark directly. The chain is:</p>
  <pre><code>mark → assessment (linked to a competency) → competency → mastery</code></pre>
  <ol class="steps">
    <li>An assessment is <strong>linked to a competency</strong> when it is created — the
    competency dropdown on the Bulk Upload Marks screen's new-assessment form.</li>
    <li>Every mark recorded against that assessment then counts toward that competency.
    A learner's level in a competency is their average percentage across its linked
    assessments.</li>
    <li>That level, against the threshold, grades the learner's heatmap cell low, medium,
    or high — and the <strong>Mastery Rate</strong> tile is the share of assessed cells
    graded high.</li>
  </ol>
  <div class="callout warn">
    <div class="t">"none linked" on the Competencies screen</div>
    <p>A competency with no assessment linked to it can never be measured: no mark has a
    path to it, its heatmap column stays unassessed, and the Competencies screen calls it
    out as <em>none linked</em>. The fix is to link an assessment — create one against it
    on Bulk Upload Marks — not to re-mark anything.</p>
  </div>
  <p>Two more honest states on the Mastery Rate tile: where a scope has marks but
  <strong>no competency evidence</strong>, the tile falls back to grading learners by their
  averages and says so beneath the figure; where there are no marks either, it shows
  <strong>—</strong> rather than a fake zero, because nothing measured is not a score of
  nothing.</p>

  <h3>Portfolio evidence</h3>
  <p>Alongside marks, learners upload <strong>portfolio evidence</strong> — files proving
  a competency in practice — which a trainer can verify. <strong>Portfolio
  Completion</strong> counts evidence submitted against the competencies a course requires;
  where no requirement is defined, it says so rather than reporting 0%.</p>

  <h3>Attendance</h3>
  <p>Attendance comes from two registers that are counted together: the live
  <strong>QR check-in sessions</strong> trainers run in class, and the
  <strong>manual register</strong> for everything the QR flow missed. Present and late both
  count as attended; every session run for a subject a learner takes counts as a sitting
  they were expected at.</p>
</section>
...
<section>
  ...
  <p>The <strong>Academic Progress Overview</strong> opens with five tiles — average score,
  enrolled subjects, mastery rate, attendance rate, unread alerts — and reads them for you:
  <strong>Learning Pulse</strong> says where you stand, <strong>Next Best Actions</strong>
  says what to do about it. Below sit your term trend, subject performance map, recent
  results, heatmaps, and announcements.</p>
  <p><strong>Your Focus Areas</strong> names the competencies you are still below target
  in and what to concentrate on next — revise this, attend more consistently — generated
  from your own marks, attendance, and competency levels. <strong>Portfolio
  Tracking</strong> beside it shows the evidence still outstanding.</p>
...
<section>
  ...
  at-risk learners, the competency heatmap, and the cohort comparison all follow it.
  <strong>Instructional Recommendations</strong> turns the same signals into teaching
  actions — which competency to re-teach, and whether group remediation is due — and the
  <strong>Practical Assessment Score Sheet</strong> lists what you have entered so you can
  check it before release. Report and analysis filters across the portal offer only your
  own subjects and courses, because that is all the data behind them covers.</p>
  ...
  <ol class="steps">
    <li><strong>Select Assessment.</strong> Pick the assessment the marks belong to — or
    create one here, choosing the <strong>competency</strong> it evidences so the marks
    count toward mastery (section 3) — then <strong>Download Class List</strong>, an Excel
    file already filled in with your learners and the codes the importer needs. Its
    assessment code is shown with a <em>Copy Code</em> button.</li>
  ...
  <ul>
    <li><strong>Practical Assess.</strong> — the Practical Assessment Platform: author
    CDACC-style practical assessments (a template loads the standard task and oral-question
    layout), mark tasks and orals per candidate, and move each record from
    <em>draft</em> to <em>complete</em> to <em>released</em>. The workspace badge names
    your institution. These are internal formative records — never upload KNEC, CDACC, or
    other externally owned summative scripts.</li>
    ...
    <li><strong>Competencies</strong> — the framework your assessments evidence, shown
    view-only: definitions and thresholds are maintained by your administrator, and the
    screen also shows which competencies still have <em>none linked</em> — no assessment
    feeding them marks (section 3 explains the fix).</li>
  </ul>
...
<section>
  ...
  <p><strong>Competencies</strong> is where the mastery framework is authored: each
  competency belongs to a module and carries the threshold learners are graded against.
  This is an administrator's screen — trainers see it read-only — and the Mastery Rate and
  Portfolio Completion tiles across the system report nothing until competencies exist and
  assessments are linked to them (section 3). The screen flags any competency with
  <em>none linked</em>, and refuses to delete one that assessments or evidence still point
  at.</p>
...
<section>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr><td><strong>Mastery Rate shows "—" or looks too low</strong></td>
        <td>Mastery is measured per competency, through linked assessments. "—" means nothing
        is measured yet; a low figure with few competencies covered usually means assessments
        are not linked. Check the Competencies screen for <em>none linked</em> — section 3
        explains the chain.</td></tr>
      <tr><td><strong>A competency reads "none linked"</strong></td>
        <td>No assessment feeds it, so no mark can count toward it. Create an assessment
        against it on Bulk Upload Marks (the new-assessment form has a competency dropdown).
        Marks recorded from then on flow to it automatically.</td></tr>
    </tbody>
  </table>
...
<section>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr><td><strong>Competency</strong></td><td>A skill a learner must evidence, defined on a module with a mastery threshold. Marks reach it through linked assessments; portfolio evidence is uploaded against it directly.</td></tr>
      <tr><td><strong>Mastery threshold</strong></td><td>The percentage on a competency that counts as having mastered it. A learner's average across the competency's linked assessments is graded against it.</td></tr>
      <tr><td><strong>Mastery Rate</strong></td><td>The share of assessed learner-by-competency cells graded high. Falls back to grading by averages where a scope holds no competency evidence, and shows "—" where nothing is measured.</td></tr>
      <tr><td><strong>Portfolio evidence</strong></td><td>A file a learner uploads to prove a competency in practice, verifiable by a trainer. Portfolio Completion counts these against what the course requires.</td></tr>
